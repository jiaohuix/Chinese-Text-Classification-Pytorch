...
DATA_FILE = "./data/corpus.jsonl"
FILTERED_OUTPUT_FILE = "data/filtered_data.jsonl"
SAMPLED_OUTPUT_FILE = "data/sample_data_500.jsonl"
BERT_OUTPUT_FILE = "data/sample_data_bert.jsonl"
...
RANDOM_SEED = 42
NUM_PROCESSES = 16
NUM_PROCESSES = max(1, (os.cpu_count() - 1)//2) # 根据你的CPU核心数调整
...
def convert_bert_data(example):
    text_labels = [chr(i) for i in range(ord('A'), ord('A')+26)]
    lbl_map =  {lbl:idx for idx, lbl in enumerate(text_labels)}
    ipt = example["human"] 
    out = example["assistant"]
    label = lbl_map[out]

    return {"text": ipt, "text_label":out, "label":int(label)}
...
def main():
    ...
    # 保存采样的bert训练的数据
    sampled_ds_bert = sampled_ds.map(convert_bert_data,remove_columns=sampled_ds.column_names)

    save_dataset_to_jsonl(sampled_ds_bert, BERT_OUTPUT_FILE, FORCE_ASCII)
