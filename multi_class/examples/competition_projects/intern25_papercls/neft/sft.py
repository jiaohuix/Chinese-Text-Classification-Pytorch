...
def NEFTune(model, noise_alpha=5):
    # neftuning ，参考https://zhuanlan.zhihu.com/p/662024086
    def noised_embed(orig_embed, noise_alpha):
        original_forward = orig_embed.forward.__func__ if hasattr(orig_embed.forward, '__func__') else orig_embed.forward
        
        def new_func(self, x):  # 注意这里增加了self参数
            if model.training:
                embed_init = original_forward(self, x)  # 传递self
                dims = torch.tensor(embed_init.size(1) * embed_init.size(2))
                mag_norm = noise_alpha / torch.sqrt(dims)
                # print("mag_norm",mag_norm)
    ...
    embed_layer = model.base_model.model.model.tok_embeddings

    
    # 替换forward方法
    embed_layer.forward = noised_embed(embed_layer, noise_alpha).__get__(embed_layer, type(embed_layer))
...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def run(self):
        ...
        # --- NEFTune Configuration and Application ---

        # Read USE_NEFTUNE from environment variable, default to 'False'
        use_neftune_env = os.environ.get('USE_NEFTUNE', 'False').lower()
        use_neftune = use_neftune_env == 'true'

        if use_neftune:
            logger.info("NEFTune is enabled. Configuring and applying...")

            # Read NEFTUNE_ALPHA from environment variable, default to '1.0'
            noise_alpha_from_env = os.environ.get('NEFTUNE_ALPHA', '1.0')
            try:
                noise_alpha = float(noise_alpha_from_env)
                # Optionally, you might want to add a check for valid noise_alpha range if needed
                if noise_alpha < 0:
                    logger.warning(f"NEFTUNE_ALPHA value ({noise_alpha}) is negative. NEFTune typically uses non-negative values. Using absolute value.")
                    noise_alpha = abs(noise_alpha)
            except ValueError:
                logger.warning(f"Invalid value for NEFTUNE_ALPHA environment variable: '{noise_alpha_from_env}'. Using default value 1.0.")
                noise_alpha = 1.0

            # Apply NEFTune with the determined noise_alpha
            self.model = NEFTune(self.model, noise_alpha=noise_alpha)

            # Log NEFTune configuration parameters
            logger.info(f"[NEFTune Config] USE_NEFTUNE: {use_neftune}, NEFTUNE_ALPHA: {noise_alpha}")
        else:
            logger.info("NEFTune is disabled (USE_NEFTUNE environment variable is not 'true').")

        # --- End of NEFTune Configuration and Application ---
