...
def NEFTune_BERT(model, noise_alpha=5):
    """为BERT模型添加嵌入层噪声的NEFTune实现"""
    def noised_embed(orig_embed, noise_alpha):
        original_forward = orig_embed.forward
        
        def new_forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, **kwargs):
            # 直接调用原始forward获取嵌入，不要传递self
            embeddings = original_forward(input_ids=input_ids, 
            ...
            if model.training:
                # 计算噪声幅度（按隐藏维度缩放）
                dims = torch.tensor(embeddings.size(-1))
    ...
    # 替换forward方法，这里使用types.MethodType来正确绑定方法
    import types
    embed_layer.forward = types.MethodType(noised_embed(embed_layer, noise_alpha), embed_layer)
...
class EnhancedTrainingArguments(TrainingArguments):
    ...
    ### 添加NEFTune参数 2025/0625
    neftune_enabled: Optional[bool] = field(
        default=False,
        metadata={"help": "是否启用NEFTune"}
    )
    neftune_alpha: Optional[float] = field(
        default=5,
        metadata={"help": "NEFTune的噪声强度"}
    )
...
def get_trainer(model, args, train_dataset=None, eval_dataset=None, 
    ...
    # 添加NEFTune支持
    if args.neftune_enabled:
        model = NEFTune_BERT(model, args.neftune_alpha)
        logger.info(f"Using NEFTune with alpha={args.neftune_alpha}")

    use_rdrop = args.rdrop_alpha > 0
    use_fgm = args.fgm_enabled
    if use_rdrop and use_fgm:
